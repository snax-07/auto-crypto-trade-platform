...
import { Plus, Trash2, TrendingUp, BarChart3, History, ChevronRight, LayoutGrid, Zap, AlertTriangle, Info, Clock, ArrowRightLeft } from 'lucide-react';
import TradePage from '@/components/tradingCharts/tradingCharts';
import { toast } from 'sonner';
import { MarketData, useTrade } from '@/hooks/useTrade';
...
export default function Page() {
  ...
  const [limitPrice, setLimitPrice] = useState("64241.50");
  ...
  const [botFields, setBotFields] = useState<BotField[]>([
    { id: '1', label: 'Investment Amount', val: '1000' }
  ]);
...
const handleSubmit = async (side: 'BUY' | 'SELL') => {
        ...
        <div className="flex flex-col flex-grow gap-3 min-w-0">
          ...
          <div className="h-60 bg-white border border-gray-200 rounded-2xl shadow-sm flex flex-col overflow-hidden">
            <div className="flex border-b border-gray-50 px-6 gap-8">
              ...
              <button className="py-4 text-[10px] font-bold text-gray-300 uppercase tracking-widest">Open Orders</button>
            </div>
  ...
  {executionOrder.length > 0 ? (
    executionOrder.map((order, i) => (
      <tr key={order._id || i} className="border-b border-gray-50 hover:bg-gray-50 transition-colors">
        {/* Time - Using a placeholder since Binance usually provides a timestamp */}
        ...
        {/* Side - Dynamic color for BUY/SELL */}
        <td className="py-3">
        ...
        {/* Price - Fallback if price is nested in your response */}
        <td className="py-3 font-mono">
          {order.actionowner ? `${(order.actionowner).toLocaleString()}` : 'Manual'}
        </td>
        
        {/* Total - Cumulative Quote Qty */}
        <td className="py-3 font-mono text-black">
          {order.total ? `${Number(order.total).toFixed(2)} USDT` : '-'}
        </td>
        <td className="py-3 font-mono text-black">
    ...
    <tr>
      <td colSpan={5} className="py-20 text-center text-gray-300 uppercase tracking-widest font-black text-[10px]">
        No recent activity
      </td>
    </tr>
  )}
              ...
              <div className="space-y-6">
                ...
                <div className="relative min-h-[300px]">
                  ...
                  <div className={`space-y-6 ${selectedStrategy !== 'swing' ? 'opacity-20 pointer-events-none' : ''}`}>
                    ...
                    <button className="w-full bg-black text-white py-5 rounded-2xl font-black uppercase tracking-[0.2em] text-[10px] shadow-xl">Launch Strategy</button>
                  </div>
                </div>
              </div>
            )}
